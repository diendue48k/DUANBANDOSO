    ...
    >
      ...
      >
        <header className="p-4 border-b border-slate-700 flex justify-between items-center flex-shrink-0 gap-4">
          <h2 className="text-2xl font-bold text-cyan-400 truncate" title={site.site_name}>{site.site_name}</h2>
          <div className="flex items-center gap-2 flex-shrink-0">
            {onShowDirections && (
                <button
                    onClick={() => onShowDirections(site)}
                    className="flex items-center gap-x-2 bg-sky-500 hover:bg-sky-600 text-white px-3 py-2 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 transform hover:-translate-y-px"
                >
                    <DirectionsIcon className="h-5 w-5" />
                    <span className="font-semibold text-sm hidden sm:inline">Chỉ đường</span>
                </button>
            )}
            <button 
              onClick={onClose}
